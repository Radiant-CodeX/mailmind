...
interface TriageStreamingPanelProps {
  /** Number of emails actually being triaged by the LLM right now (cache hits excluded). */
  count: number;
}

export function TriageStreamingPanel({ count }: TriageStreamingPanelProps) {
  // Only shown while real LLM triage work is in flight — disappears when done.
