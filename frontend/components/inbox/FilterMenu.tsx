    ...
    <div className="relative" ref={ref}>
      <button
        ...
        className={`flex items-center gap-1 p-1.5 rounded-md hover:bg-[var(--bg-elevated)] transition-all cursor-pointer ${
          activeCount ? 'text-[var(--accent-primary)]' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'
        }`}
        title="Filters"
      ...
      >
        ...
        {activeCount > 0 && (
          <span className="text-[9px] font-bold bg-[var(--accent-primary)] text-[var(--bg-surface)] rounded-full w-4 h-4 flex items-center justify-center">
            {activeCount}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 mt-1.5 w-60 bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl shadow-2xl p-3 z-50 animate-fade-in">
          <div className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)] mb-2">Date</div>
              ...
              <button
                ...
                className={`px-2 py-1.5 rounded-lg text-[11px] font-semibold transition-all cursor-pointer ${
                  value.dateRange === opt.key
                    ...
                    : 'bg-[var(--bg-elevated)] text-[var(--text-primary)] hover:bg-[var(--bg-elevated)]/70'
                }`}
              >
                {opt.label}
              </button>
          ...
          <div className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)] mb-2">Show only</div>
          <label className="flex items-center gap-2 px-1 py-1.5 cursor-pointer text-xs text-[var(--text-primary)]">
          ...
          {activeCount > 0 && (
            <button
              ...
              className="mt-2 w-full text-[11px] font-bold text-[var(--text-muted)] hover:text-red-500 transition-colors cursor-pointer py-1"
            >
              Clear filters
            </button>
          )}
