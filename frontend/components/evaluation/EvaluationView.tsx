...
function PriorityChip({ label }: { label: string }) {
  const cls = PRIORITY_COLORS[label] ?? 'text-base-content/60 bg-base-200 border-base-300';
  return (
...
export function EvaluationView() {
  ...
  const accuracyColor =
    !data ? 'text-base-content/60'
    : data.accuracy >= 80 ? 'text-emerald-500'
    : data.accuracy >= 60 ? 'text-amber-500'
    : 'text-red-500';

  return (
    <div className="flex-1 h-full overflow-y-auto bg-base-300 p-6 custom-scrollbar" id="evaluation-view">
      {/* Header */}
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={runEval}
            disabled={loading}
            className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 disabled:opacity-50 text-base-100 font-bold text-sm rounded-lg cursor-pointer transition-all active:scale-95 shadow"
            id="btn-run-eval"
          >
            {loading ? (
              <span className="w-4 h-4 rounded-full border-2 border-base-100 border-t-transparent animate-spin" />
            ) : (
        ...
        {!data && !loading && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-sm font-semibold text-base-content">No results yet</p>
            <p className="text-xs text-base-content/60 mt-1">Click Run Evaluation to benchmark the triage classifier.</p>
          </div>
        )}
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="text-xs text-base-content/60 font-medium">Running classifier against golden dataset…</p>
          </div>
        )}
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-base-100 border border-base-300 rounded-xl p-4 text-center">
                <p className="text-[10px] text-base-content/60 uppercase tracking-widest font-bold mb-1">Accuracy</p>
              ...
              {/* Correct */}
              <div className="bg-base-100 border border-base-300 rounded-xl p-4 text-center">
                <p className="text-[10px] text-base-content/60 uppercase tracking-widest font-bold mb-1">Correct</p>
                <p className="text-3xl font-black text-emerald-500 tabular-nums">{data.correct_predictions}</p>
                <p className="text-[10px] text-base-content/60 mt-1 font-medium">out of {data.total_samples} samples</p>
              </div>

              {/* Wrong */}
              <div className="bg-base-100 border border-base-300 rounded-xl p-4 text-center">
                <p className="text-[10px] text-base-content/60 uppercase tracking-widest font-bold mb-1">Incorrect</p>
                <p className="text-3xl font-black text-red-500 tabular-nums">
                  {data.total_samples - data.correct_predictions}
                </p>
                <p className="text-[10px] text-base-content/60 mt-1 font-medium">misclassified</p>
              </div>
            </div>
            ...
            <div className="bg-base-100 border border-base-300 rounded-xl overflow-hidden">
              <div className="grid grid-cols-[1fr_120px_120px_40px] gap-0 text-[10px] font-bold text-base-content/60 uppercase tracking-widest border-b border-base-300 px-4 py-2.5 bg-base-200">
                <span>Subject</span>
                <span>Expected</span>
                <span>Predicted</span>
              ...
              {filtered.length === 0 && (
                <div className="py-8 text-center text-xs text-base-content/60 font-medium">
              ...
              {filtered.map((r, i) => (
                <div
                  key={i}
                  className={`grid grid-cols-[1fr_120px_120px_40px] gap-0 items-center px-4 py-2.5 border-b border-base-200 last:border-b-0 ${
                    r.is_correct ? '' : 'bg-red-500/5'
                  }`}
                >
                  <span className="text-xs text-base-content font-medium truncate pr-4"
                    title={r.subject}>
                    {r.subject}
                  </span>
