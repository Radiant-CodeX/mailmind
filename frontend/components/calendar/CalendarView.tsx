...
function fmt(iso: string, opts: Intl.DateTimeFormatOptions) {
  try { return new Date(iso).toLocaleString(undefined, opts); } catch { return ''; }
}
...
export function CalendarView({ events, loading, error, onRefresh, onCreateEvent }: CalendarViewProps) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(true);
    try {
      await onCreateEvent({ title: newTitle, start_time: newStart, end_time: newEnd || newStart, organizer: '' });
      setShowCreate(false);
  ...
  // ── Render helpers ────────────────────────────────────────────────────────
  function EventPill({ event, idx, compact = false }: { event: CalendarEvent; idx: number; compact?: boolean }) {
    return (
      <button
        ...
        title={event.title}
      >
        {!compact && <span className="mr-1 opacity-70">
  ...
  return (
    <div className="flex-1 bg-[var(--bg-base)] flex flex-col h-full overflow-hidden text-left" id="calendar-view">
        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            {(['month','week','agenda'] as ViewMode[]).map(v => (
              <button key={v} onClick={() => setView(v)}
                className={`px-3 py-1.5 capitalize transition-colors cursor-pointer
                  ${view === v ? 'bg-[var(--accent-primary)] text-white' : 'bg-[var(--bg-surface)] text-[var(--text-muted)] hover:bg-[var(--bg-elevated)]'}`}>
                {v}
              </button>
            ))}
          </div>
          ...
          {view !== 'agenda' && (
            <div className="flex items-center gap-1">
              <button onClick={prevPeriod} className="w-7 h-7 flex items-center justify-center rounded bg-[var(--bg-surface)] border border-[var(--border)] hover:bg-[var(--bg-elevated)] cursor-pointer text-[var(--text-primary)]">‹</button>
              <button onClick={goToday} className="px-2 h-7 text-xs font-bold rounded bg-[var(--bg-surface)] border border-[var(--border)] hover:bg-[var(--bg-elevated)] cursor-pointer text-[var(--text-primary)]">Today</button>
              <button onClick={nextPeriod} className="w-7 h-7 flex items-center justify-center rounded bg-[var(--bg-surface)] border border-[var(--border)] hover:bg-[var(--bg-elevated)] cursor-pointer text-[var(--text-primary)]">›</button>
            </div>
          )}

          <span className="text-sm font-bold text-[var(--text-primary)] min-w-[180px] text-center">{periodLabel}</span>

          {/* Create + Refresh */}
          {onCreateEvent && (
            <button onClick={() => setShowCreate(true)}
              className="px-3 h-7 text-xs font-bold rounded bg-[var(--accent-primary)] text-white hover:opacity-90 cursor-pointer flex items-center gap-1">
          ...
          <button onClick={onRefresh} disabled={loading}
            ...
            {loading ? <span className="w-3 h-3 border border-[var(--text-muted)] border-t-[var(--text-primary)] rounded-full animate-spin" /> : '↻'}
      ...
      <div className="px-6 py-2 flex gap-6 border-b border-[var(--border-subtle)] shrink-0">
        {[
          ...
          { label: 'Today', value: events.filter(e => { try { return isSameDay(new Date(e.start_time), today); } catch { return false; } }).length },
          { label: 'This Week', value: events.filter(e => { try { const d = new Date(e.start_time); const diff = (d.getTime() - today.getTime()) / 86400000; return diff >= 0 && diff < 7; } catch { return false; } }).length },
          { label: 'Sync Status', value: loading ? 'Syncing…' : error ? 'Error' : 'Live' },
        ].map(s => (
          ...
          {loading && events.length === 0 ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <p className="text-xs text-[var(--text-muted)]">Fetching calendar from Microsoft Graph…</p>
              </div>
            </div>
          ) : error ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center max-w-sm space-y-3">
                ...
                <p className="text-sm font-semibold text-[var(--text-primary)]">Calendar unavailable</p>
                <p className="text-xs text-[var(--text-muted)]">{error}</p>
                <p className="text-xs text-[var(--text-muted)]">Sign in with a Microsoft account to sync Outlook calendar.</p>
                <button onClick={onRefresh} className="px-4 py-2 text-xs font-bold bg-[var(--accent-primary)] text-white rounded-lg hover:opacity-90 cursor-pointer">
                  Retry
                </button>
              </div>
            </div>
            ...
            <div className="min-w-[560px]">
              ...
              <div className="grid grid-cols-7 mb-1">
                {WEEKDAYS.map(d => (
                  <div key={d} className="py-1 text-center text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider">{d}</div>
                ))}
              </div>
              {/* Day cells */}
                ...
                {monthDays.map((day, i) => {
                  ...
                  return (
                    <div key={i}
                      ...
                      {day && (
                        <>
                          <span className={`text-xs font-bold mb-1 self-start w-6 h-6 flex items-center justify-center rounded-full
                            ${isToday ? 'bg-[var(--accent-primary)] text-white' : 'text-[var(--text-muted)]'}`}>
                            {day.getDate()}
                          </span>
                          <div className="space-y-0.5 overflow-hidden">
                            {dayEvents.slice(0, 3).map((ev, idx) => (
                              <EventPill key={idx} event={ev} idx={events.indexOf(ev)} compact />
                            ))}
                            {dayEvents.length > 3 && (
            ...
            <div className="min-w-[560px]">
              <div className="grid grid-cols-7 gap-px bg-[var(--border-subtle)] rounded-lg overflow-hidden border border-[var(--border-subtle)]">
                {weekDays.map((day, i) => {
                  ...
                  return (
                    <div key={i} className={`flex flex-col bg-[var(--bg-surface)] min-h-[300px]
                      ${isToday ? 'ring-1 ring-inset ring-[var(--accent-primary)]' : ''}`}>
                      {/* Day header */}
                      <div className={`p-2 text-center border-b border-[var(--border-subtle)]
                        ${isToday ? 'bg-[var(--accent-primary)]/10' : ''}`}>
                        <div className="text-[10px] font-bold text-[var(--text-muted)] uppercase">{WEEKDAYS[day.getDay()]}</div>
                      ...
                      <div className="p-1.5 space-y-1 flex-1">
                        {dayEvents.length === 0 ? (
                          ...
                          dayEvents.map((ev, idx) => (
                            <EventPill key={idx} event={ev} idx={events.indexOf(ev)} />
                          ))
                        )}
                      </div>
                    </div>
            ...
            <div className="max-w-2xl space-y-2">
              {sortedEvents.length === 0 ? (
                <div className="py-16 text-center">
                  ...
                  <p className="text-sm font-semibold text-[var(--text-primary)]">No upcoming events</p>
                  <p className="text-xs text-[var(--text-muted)] mt-1">Your calendar is clear for the next 7 days.</p>
                </div>
              ) : (
                (() => {
                  let lastDate = '';
                  return sortedEvents.map((event, idx) => {
                    const dateLabel = fmt(event.start_time, { weekday: 'long', month: 'long', day: 'numeric' });
                    const showDateHeader = dateLabel !== lastDate;
                    lastDate = dateLabel;
                    const isToday = isSameDay(new Date(event.start_time), today);
                    return (
                      <React.Fragment key={idx}>
                        {showDateHeader && (
                          <div className={`pt-4 pb-1 text-xs font-bold uppercase tracking-wider
                        ...
                        <button
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-start gap-3 min-w-0">
                              <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${eventColor(idx).split(' ')[0].replace('bg-', 'bg-').replace('/20', '/80')}`} />
                              <div className="min-w-0">
                                <p className="text-sm font-semibold text-[var(--text-primary)] truncate">{event.title}</p>
                                <p className="text-xs text-[var(--text-muted)] mt-0.5">
                                  {event.organizer && event.organizer !== 'unknown@example.com' ? `By ${event.organizer}` : ''}
                                </p>
                              </div>
                            </div>
        ...
        {selected && (
          <div className="w-72 border-l border-[var(--border-subtle)] bg-[var(--bg-surface)] p-5 flex flex-col gap-4 shrink-0 overflow-y-auto">
            <div className="flex items-start justify-between">
              <h3 className="text-sm font-bold text-[var(--text-primary)] leading-snug flex-1 pr-2">{selected.title}</h3>
              <button onClick={() => setSelected(null)} className="text-[var(--text-muted)] hover:text-[var(--text-primary)] cursor-pointer text-lg leading-none">×</button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 rounded-lg bg-[var(--bg-base)] border border-[var(--border-subtle)] space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-[var(--text-primary)]">
                    {fmt(selected.start_time, { weekday: 'short', month: 'short', day: 'numeric' })}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">Start</span>
                  <span className="font-mono font-semibold text-[var(--text-primary)]">
                    {fmt(selected.start_time, { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">End</span>
                  <span className="font-mono font-semibold text-[var(--text-primary)]">
                    {fmt(selected.end_time, { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
                <div className="flex justify-between">
              ...
              {selected.organizer && selected.organizer !== 'unknown@example.com' && (
                <div className="p-3 rounded-lg bg-[var(--bg-base)] border border-[var(--border-subtle)]">
                  <div className="text-[var(--text-muted)] mb-1">Organizer</div>
                  <div className="font-semibold text-[var(--text-primary)] break-all">{selected.organizer}</div>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-[var(--text-primary)]">New Calendar Event</h3>
              <button onClick={() => setShowCreate(false)} className="text-[var(--text-muted)] hover:text-[var(--text-primary)] cursor-pointer text-lg">×</button>
            </div>
            ...
            <form onSubmit={handleCreate} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-muted)] mb-1">Start *</label>
                  <input
                    type="datetime-local" value={newStart} onChange={e => setNewStart(e.target.value)} required
                ...
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-muted)] mb-1">End</label>
                  <input
                    type="datetime-local" value={newEnd} onChange={e => setNewEnd(e.target.value)}
                    className="w-full px-3 py-2 rounded-lg bg-[var(--bg-elevated)] border border-[var(--border)] text-sm text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)]"
                  />
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button type="submit" disabled={creating || !newTitle || !newStart}
                  ...
                  {creating ? <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : null}
